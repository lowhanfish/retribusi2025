<template>
  <q-layout view="lHh Lpr lFf">
    <q-header elevated class="glossy bg-purple-3" v-if="!checkLogin">
      <q-toolbar>
        <q-btn flat dense round @click="leftDrawerOpen = !leftDrawerOpen" aria-label="Menu" icon="menu" />
        <q-toolbar-title>
          Aplikasi Contoh Template (Versi Admin)
        </q-toolbar-title>
        <div>
          <q-btn style="color:white" flat color="white" @click="$q.fullscreen.toggle()" :icon="$q.fullscreen.isActive ? 'fullscreen_exit' : 'fullscreen'" />
        </div>
      </q-toolbar>
    </q-header>

<template>
  <q-list>
    <q-img src="img/card.png" style="width: 100%">
      <div class="absolute-bottom text-subtitle1">Ver 1.0.0</div>
    </q-img>

    <q-item-label class="bg-purple-3" header><span style="color:white">{{ nama }}</span></q-item-label>
      ...
      <q-expansion-item v-if="checkLength(data.subItem)" :key="data.index">
        ...
        <q-list bordered>
          ...
          <template v-for="(data1, index1) in data.subItem">
            ...
            <q-expansion-item :key="index1" v-if="checkLength(data1.subItem)" class="h_sidebar_sub_menu"
              expand-separator :content-inset-level="1">
...
export default {
  ...
  methods: {
    ...
    async loadMenu() {
      ...
      var dataMenu = await this.FETCHING.postMasterMenuGetSideBar(profileku.retribusi)
